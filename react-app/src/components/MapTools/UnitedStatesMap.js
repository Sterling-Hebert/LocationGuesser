...
import { UnitedStatesPlaces } from "./places.js";
import {
  GoogleMap,
  LoadScript,
  StreetViewPanorama,
...
function UnitedStatesMap() {
  ...
  const generateNewPosition = () => {
    let pos = UnitedStatesPlaces[Math.floor(Math.random() * UnitedStatesPlaces.length)];
    while (positions.includes(pos)) {
      pos = UnitedStatesPlaces[Math.floor(Math.random() * UnitedStatesPlaces.length)];
    }
    return pos;
  };
